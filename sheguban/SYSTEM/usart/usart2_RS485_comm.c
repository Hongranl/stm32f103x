		 	        ...
		 	        switch(  fuc_dat[0]<<8|fuc_dat[1]  ) 
							{
									...
									case  0x0002:   //获得上一级  设备识别号 和地址 
									    if(fuc_dat[3]==DTN_main_manage) //识别号为 DTN_main_manage 返回的
											{
									     main_manage_DEST_ADDR=fuc_dat[4]|fuc_dat[5]<<8| fuc_dat[6]<<16|fuc_dat[7]<<24;
												search_addr_flg=1;
											}
										break;
									default:  break; 
			        ...
			        switch(  fuc_dat[0]<<8|fuc_dat[1]  ) 
							{
									...
									case  0x0002:     //查询电源管理器 电压值和电流值 CMD 0X0002  反馈 电压值2字节 电流值2字节， 瓦特2字节，电能4字节
									    if( TRAN_info1.source_dev_num   ==DTN_main_manage) //识别号为 DTN_main_manage  被查询
											{  
															
													      TRAN_info2.dest_dev_num= (DTN_main_manage<<8|DTN_main_manage>>8);
							  
														   
															  if( main_manage_DEST_ADDR!=0 )
																 {
																 TRAN_info2.dest_addr[0]=main_manage_DEST_ADDR;
																 TRAN_info2.dest_addr[1]=(u8)(main_manage_DEST_ADDR>>8);
																 TRAN_info2.dest_addr[2]=(u8)(main_manage_DEST_ADDR>>16);
																 TRAN_info2.dest_addr[3]=(u8)(main_manage_DEST_ADDR>>24);
								 ...
								 // 收到命令后，反馈 一个字节表示 1开和 0关，2没有电
									...
									if( TRAN_info1.source_dev_num ==DTN_main_manage)//  
								 {
