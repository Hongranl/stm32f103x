	...
	switch (EXmodID)
		{
			...
			case MID_SENSOR_TEMP:{
						...
						memcpy(sendbuff+16+updataTRAN.data_len,gb_databuff, 8);
						evt = osMessageGet(MsgBox_tempMS, 10);
						if (evt.status == osEventMessage) {		//等待消息指令
							...
							updataTRAN.data_len = 8+16;	
							//memcpy(gb_databuff,&rptr->temp, 4);
							//memcpy(gb_databuff+4,&rptr->hum, 4);
							//gb_databuff[0]=22;gb_databuff[3]=33;
			...
			case MID_SENSOR_LIGHT:{
						...
						if (evt.status == osEventMessage) {		//等待消息指令
							...
							memcpy(sendbuff+16+updataTRAN.data_len, &(rptr->illumination), 1);//17+updataTRAN.data_len-1
