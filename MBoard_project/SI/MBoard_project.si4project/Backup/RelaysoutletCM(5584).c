...
osThreadId tid_soutlet_key_Thread;
osThreadDef(soutlet_key_Thread,osPriorityNormal,1,256);
...
void RelaysoutletCM_Thread(const void *argument){
	...
	for(;;){
		...
		if(key_vlue == 0)
		{
			...
			if(key_vlue == 0)
				{
					outlet1 	= light1_vlue =!light1_vlue;
					outlet1_led	=!light1_vlue;
...
void soutlet_key_Thread(const void *argument)
{
	while(1)
	{
...
void RelaysoutletCMThread_Active(void){
	...
	tid_soutlet_key_Thread    = osThreadCreate(osThread(soutlet_key_Thread),NULL);
}
