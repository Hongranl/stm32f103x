...
void curtainCM_Thread(const void *argument){
	...
	for(;;){
		...
		if(!curtainENABLE){			
			...
			Kcnt = 100;
			while(!curtainENABLE && Kcnt){
				osDelay(20);Kcnt --;
			}
			actuatorData.valACT  = 0x4;
		}
		else if(!curtKeySTP){			
		...
		if((actuatorData.valACT != valACT_NULL) && !sigOut_FLG){
			...
			switch(actuatorData.valACT){
				...
				case CMD_CURTUP:	curtconDN = 0; delay_ms(20);curtconUP  = 1;delay_ms(50);	break;
				...
				case CMD_CURTDN:	curtconUP = 0; delay_ms(20);	curtconDN  = 1;	delay_ms(50);break;
				default: 
					curtconUP = 0;
					curtconDN = 0;
					curtconSTP = 1;
