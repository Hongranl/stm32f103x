#include "shakeMS.h"//火焰传感器检测驱动的进程函数
...
void shakeMS_Thread(const void *argument){
	...
	/***********************进程数据推送*****************************************************/		
		if(UPLD_EN){
			...
			do{mptr = (shakeMS_MEAS *)osPoolCAlloc(shakeMS_pool);}while(mptr == NULL);	//无线数据传输消息推送
			mptr->VAL = sensorData.VAL;
			osMessagePut(MsgBox_shakeMS, (uint32_t)mptr, 100);
			osDelay(500);
		}
