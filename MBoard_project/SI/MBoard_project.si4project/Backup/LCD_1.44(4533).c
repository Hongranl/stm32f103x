...
void LCD144Disp_Thread(const void *argument){
	...
	Show_Str(5,50,YELLOW,BLACK,"这是测试",24,1);//未检测到
	Show_Str(25,75,YELLOW,BLACK,"这是测试",24,1);//扩展模块
...
//							}
			...
			case MID_SENSOR_GAS:{
						...
						if (evt.status == osEventMessage){
							...
							sprintf(disp,"anaVAL:%d0mV",rptr->anaDAT);
							Show_Str(10,105,BLUE,BLACK,(uint8_t *)disp,24,1);
			...
			case MID_EXEC_RELAYSOUTLET:{
						...
						if (evt.status == osEventMessage){
							...
							sprintf(disp,"I = %dA",rptr->Current);
			...
			case MID_EXEC_SOURCE:{
						...
						sourceCM_MEAS *rptr;
						osEvent  evt;
						char disp[30];unsigned char send_buff[25]	= {0x68 ,
									0,0,0,0,0,0,
									0x68,
									0x11,0x04,
									0x00,0x00,0x00,0x00,
...
//							rptr->power_i = 990;
							/*显示部分程序↓↓↓↓↓↓↓*/
								...
								sprintf(disp,"%d.%.1dKWH",rptr->power_p/10,rptr->power_p%10);
								...
								else if(rptr->power_i <1000)
									sprintf(disp,"%d.%.3dA",rptr->power_i/100000,(rptr->power_i/100)%1000);
								Show_Str(20,100,GREEN,BLACK,(uint8_t *)disp,24,1);

								LCD_1_44_Clear(BLACK); //清屏
								
								sprintf(disp,"-%x-%x-%x-%x-%x",RS485_RX_BUF[0],RS485_RX_BUF[1],RS485_RX_BUF[2],RS485_RX_BUF[3],RS485_RX_BUF[4]);
								Show_Str(0,0,GREEN,BLACK,(uint8_t *)disp,12,1);
								//Show_Str(70,3,LGRAYBLUE,BLACK,"MSG_type:",12,1);
