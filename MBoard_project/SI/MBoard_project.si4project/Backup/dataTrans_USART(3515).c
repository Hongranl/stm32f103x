...
LEDstatus zigbee_init(void)
{
	uint8 sendbuff[100]={0};
	uint8 recvbuff[100]={0};
	uint8 i = 0;
	Driver_USART1.Send("---------入网状态查询",strlen("---------入网状态查询"));
	...
	if(led2_status != led2_b)
		{
			...
			while(1)
			{
				...
				{	if( i = 0)
					{
						i++;
						Driver_USART1.Send("---------未入网状态",strlen("---------未入网状态"));
					}
				}
				delay_ms(200);
			}
...
//	Moudle_GTA.Wirless_ID = 0xAA;   			/****调试语句*****/
	...
	for(;;){
		...
		memcpy(&RXdats_BUFtemp[memp],dataTransFrameHead,dataTransFrameHead_size);//填充帧头
		memp += dataTransFrameHead_size;
		RXdats_BUFtemp[memp ++] = Moudle_GTA.Wirless_ID;//填充通讯模块字节
		RXdats_BUFtemp[memp ++] = datsTransCMD_DOWNLOAD;//填充下发字节
		RXdats_BUFtemp[memp ++] = Extension_IDCHG(Moudle_GTA.Extension_ID);//填充扩展模块编码字节
		...
		if(p){
			...
			memset(RXdats_BUFtemp, 0, sizeof(u8) * dats_BUFtemp_len);//将帧头清零
...
//			osDelay(20);								/****调试语句*****/
			...
			if(!memcmp((const char*)RXdats_BUFtemp,dataTransFrameTail,dataTransFrameTail_size)){	//帧尾校验
				...
				memset(RXdats_BUFtemp, 0, sizeof(u8) * dats_BUFtemp_len);//将帧尾清零
