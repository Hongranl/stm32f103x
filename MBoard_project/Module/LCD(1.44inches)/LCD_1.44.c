 ...
 * @Module:     LCD_1.44
 * @Author:     RanHongLiang
 * @Date:       2019-07-03 10:24:42
 * @Description: 
 *————1.44屏显示模块驱动, 默认为竖屏
 *---------------------------------------------------------------------------*/

#include "LCD_1.44.h"//显示驱动进程函数；
...
/*---------------------------------------------------------------------------
 * @Description:STM32_模拟SPI写一个字节数据底层函数
 * @Param:       Data
 * @Return:     无
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
 * @Description:STM32_硬件SPI读写一个字节数据底层函数
 * @Param:      SPIx,Byte
 * @Return:     返回总线收到的数据
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
 * @Description:设置SPI的速度
 * @Param:      SPIx,SpeedSet,SpeedSet:1,高速;0,低速;
 * @Return:     无
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
 * @Description: 写寄存器数据
 * @Param:      LCD_1_44_Reg:寄存器地址,LCD_1_44_RegValue:要写入的数据
 * @Return:     无
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
 * @Description:开始写GRAM
 * @Param:      在给液晶屏传送RGB数据前，应该发送写GRAM指令
 * @Return:     无
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
 * @Description:液晶屏IO初始化，液晶初始化前要调用此函数
 * @Param:      无
 * @Return:     无
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
 * @Description:LCD复位函数，液晶初始化前要调用此函数
 * @Param:      无
 * @Return:     无
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
 * @Description:LCD填充清处函数
 * @Param:      Color 颜色，x 起始坐标，y 起始坐标，xx 偏移量， yy 偏移量。
 * @Return:     无
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
 * @Description:设置lcd显示窗口，在此区域写点数据自动换行
 * @Param:      xy起点和终点
 * @Return:     无
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
 * @Description:设置LCD参数,方便进行横竖屏模式切换
 * @Param:      无
 * @Return:     无
 *---------------------------------------------------------------------------*/
...
void LCD144Disp_Thread(const void *argument){
	...
	static char EXT_ID,MSG_ID;//,WZADDR_Disp[10],ZW_ADDR
	u8 EXTID_Disp[5],MSGID_Disp[5];
	osStatus status;
	...
	Show_Str(15,50,YELLOW,BLACK,"这是测试",24,1);//未检测到
	Show_Str(15,75,YELLOW,BLACK,"这是测试",24,1);//扩展模块
		...
		if(EXT_ID != Moudle_GTA.Extension_ID){		//检测到模块更改，基层界面更新
			...
			switch(Moudle_GTA.Extension_ID){		
				...
				case MID_SENSOR_SMOKE:
						...
						Show_Str(5,50,WHITE,BLACK,"粉尘监测鑞",24,1);
						Show_Str(50,75,WHITE,BLACK,"X X",24,1);
						break;
				...
				case MID_SENSOR_SHAKE:	
						...
						Show_Str(5,50,WHITE,BLACK,"震动监测鑞",24,1);
						Show_Str(50,75,WHITE,BLACK,"X X",24,1);
						break;
				...
				case MID_SENSOR_ANALOG:
						...
						Show_Str(5,25, WHITE,BLACK,"CH1:",24,1);
						Show_Str(5,50, WHITE,BLACK,"CH2:",24,1);
						Show_Str(5,75, WHITE,BLACK,"CH3:",24,1);
						Show_Str(5,100,WHITE,BLACK,"CH4:",24,1);
...
//						Show_Str(50,100,WHITE,BLACK,"X X",24,1);	
				...
				case MID_EXEC_RELAYSLIGHT:
						...
						Show_Str(5,25,WHITE,BLACK,"灯光开关一鑞",24,1);
						Show_Str(50,50,GREEN,BLACK,"X X",24,1);
						Show_Str(5,75,WHITE,BLACK,"灯光开关二鑞",24,1);
						Show_Str(50,100,GREEN,BLACK,"X X",24,1);
						break;
				case MID_EXEC_RELAYSOUTLET:
					
						Show_Str(5,25,WHITE,BLACK,"插座开关鑞",24,1);
						Show_Str(50,50,GREEN,BLACK,"X X",24,1);
						Show_Str(5,75,WHITE,BLACK,"电流鑞",24,1);
						Show_Str(50,100,GREEN,BLACK,"X X",24,1);
						break;
				...
				case MID_EXEC_SOURCE:
						...
						Show_Str(5,25,WHITE,BLACK,"电表状态鑞",24,1);
						Show_Str(50,50,GREEN,BLACK,"X X",24,1);
						Show_Str(50,75,GREEN,BLACK,"X X",24,1);
						Show_Str(50,100,GREEN,BLACK,"X X",24,1);	
						break;
				...
				default:
						...
						Show_Str(15,50,YELLOW,BLACK,"未检测到",24,1);
						Show_Str(15,75,YELLOW,BLACK,"扩展模块",24,1);
						break;
			}
		}
...
//							}
			...
			case MID_SENSOR_SHAKE:{
						...
						evt = osMessageGet(MsgBox_DPshakeMS, 10);
						if (evt.status == osEventMessage){
			...
			case MID_SENSOR_ANALOG:{
						...
						if (evt.status == osEventMessage){
							...
							//VOL = 420.0 / 4096.0 * (float)rptr->Vch1;		//电压
							VOL = (float)rptr->Vch1 *  5.0 / 4096.0;//V= data*3.3/4096*3/2
							//电流
							CUR = (float)rptr->Ich1 *  22.0 / 4096.0 ;//i= data*3.3*1000/4096/150
							
							VOL2 = (float)rptr->Vch2 *  5.0 / 4096.0;//V= data*3.3/4096*3/2
							//电流
							CUR2 = (float)rptr->Ich2 *  22.0 / 4096.0 ;//i= data*3.3*1000/4096/150

							LCD_1_44_ClearS(BLACK,45,50,127,75);
							LCD_1_44_ClearS(BLACK,45,100,127,125);

							LCD_1_44_ClearS(BLACK,45,25,127,75);
							LCD_1_44_ClearS(BLACK,45,75,127,125);
							
							sprintf(disp,"%3.0f mA",CUR);
							Show_Str(45,25,GREEN,BLACK,(uint8_t *)disp,24,1);
							
							sprintf(disp,"%.2f V",VOL);
							Show_Str(45,50,GREEN,BLACK,(uint8_t *)disp,24,1);
							
							sprintf(disp,"%3.0f mA",CUR2);
							Show_Str(45,75,GREEN,BLACK,(uint8_t *)disp,24,1);
							

							sprintf(disp,"%.2f V",VOL2);
							Show_Str(45,100,GREEN,BLACK,(uint8_t *)disp,24,1);
						...
						if(DP_FLG){		//是否界面自恢复
							...
							if(DP_cnt)DP_cnt --;
								...
								switch(EXD_ADDR){
									...
									case pwmDevMID_varLight:	Show_Str(5,25,WHITE,BLACK,"开关状态鑞",24,1);
																Show_Str(5,75,WHITE,BLACK,"风力鑞",24,1);
																break;
							...
							switch(rptr->speDPCMD){
								...
								case SPECMD_ifrDevModADDR_CHG:{
									...
									switch(rptr->Mod_addr){
										...
										case pwmDevMID_varLight:	Show_Str(56,14,YELLOW,BLACK,"pwm-Fan",12,1); 
																	break;
			...
			case MID_EXEC_RELAYSLIGHT:{
						...
						evt = osMessageGet(MsgBox_DPRelayslightCM, 10);
						if (evt.status == osEventMessage){
			...
			case MID_EXEC_RELAYSOUTLET:{
						...
						RelaysoutletCM_MEAS *rptr;
						osEvent  evt;
						char disp[30];
				
						evt = osMessageGet(MsgBox_DPRelaysoutletCM, 10);
						if (evt.status == osEventMessage){
							...
							sprintf(disp,"I = %.2fA",((float)rptr->Current / 185.0));
							Show_Str(40,100,RED,BLACK,(uint8_t *)disp,24,1);
			...
			case MID_EXEC_CURTAIN:{
						...
						if (evt.status == osEventMessage){
							...
							switch(rptr->valACT){
								...
								case CMD_CURTmax:	sprintf(disp,"限位");break;
...
//							rptr->power_i = 990;
							/*显示部分程序↓↓↓↓↓↓↓*/
								...
								//sprintf(disp,"%.1f KWH",rptr->power_p /10);
								sprintf(disp,"%d.%.2dkw/h",rptr->power_p/100,(rptr->power_p)%100);
								Show_Str(20,50,GREEN,BLACK,(uint8_t *)disp,24,1);
		
							
							
								LCD_1_44_ClearS(BLACK,0,75,127,127);
								sprintf(disp,"%d.%.1dV",rptr->power_v/10,rptr->power_v%10);
								Show_Str(20,75,GREEN,BLACK,(uint8_t *)disp,24,1);
								...
								sprintf(disp,"%d.%.3dA",rptr->power_i/1000,rptr->power_i%1000);
...
//										for (i = 0; i < 7; ++i)
//										{
//											sprintf(disp+(3*(i+(7 * j ))),"-%x",RS485_RX_BUF[i+(7*j)]);
//											
//										}
//										Show_Str(3,(12*j)+3,GREEN,BLACK,(uint8_t *)disp+(3*7*j),12,1);
//									}
