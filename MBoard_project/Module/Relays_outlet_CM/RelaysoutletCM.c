 ...
 * @Description: 
 * ————智能插座模块，包括电源控制，电流检测，状态上传，同步控制
 * ---------------------------------------------------------------------------*/


#include "RelaysoutletCM.h"//一路继电器驱动进程函数；
	...
	for(;;){
		...
		if(Data_tempDP.relay_con != actuatorData.relay_con
		  || Data_tempDP.Current != actuatorData.Current){
			...
			memcpy(gb_databuff+1, &actuatorData.Current, 4);
			Data_tempDP.relay_con = actuatorData.relay_con;
			Data_tempDP.Current 	= actuatorData.Current;
			light1_vlue = (Data_tempDP.relay_con >> 0) & 0x01;
		...
		{
			//actuatorData.Current = AD_temp /10;
			
			if(light1_vlue)
				AD_temp_1 = AD_temp / 10;
			else
				AD_temp_2 = AD_temp / 10;

			if(AD_temp_2 != 0 && AD_temp_1 != 0)
				actuatorData.Current = AD_temp_2 > AD_temp_1 ? AD_temp_2 - AD_temp_1 : 0;
			if(light1_vlue == 0)
				actuatorData.Current = 0;
