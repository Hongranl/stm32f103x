 ...
 * @Module:     fingerID
 * @Author:     RanHongLiang
 * @Date:       2019-06-27 09:04:31
 * @Description: 
 *————串口指纹识别模块，处理线程
 *	这里使用模块化指纹识别设备，所以全是串口指令既是类AT，详情参考文档
 * 	指纹识别命令，指纹识别存储，指纹删除命令
 * ---------------------------------------------------------------------------*/

#include "fingerID.h"//指纹扫描模块驱动进程函数；
...
/*---------------------------------------------------------------------------
 *
 * @Description:数据校验和，半字校验
 * @Param:      dats[],byte数组，u8 length数组长度
 * @Return:     
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
 *
 * @Description:将指令按格式装载进数据缓存包，返回数据包长，指令包含指令码及指令码后、校验码前所有内容
 * @Param:      u8 bufs[]数据缓存包,u8 cmd[]指令包,u16 cmdlen指令包长
 * @Return:     返回数组游标指针，即包长
 *---------------------------------------------------------------------------*/
u16 FIDframeLoad_TX(u8 bufs[],u8 cmd[],u16 cmdlen){  //
...
/*---------------------------------------------------------------------------
 *
 * @Description:按指令编号发送指令,CMD_ID为0xff时，发送动态复合指令
 * @Param:     u8 CMD_ID, 命令 ， u8 rpt_num 次数
 * @Return:     返回门禁结构体，包含门禁命令，门禁数据，门禁指纹数据，门禁密码数据
 *---------------------------------------------------------------------------*/
