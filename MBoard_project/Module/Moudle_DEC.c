			...
			if(alive_temp & wireless_FLG){
				...
				if(wireless_IF){
					...
					switch(Moudle_GTA.Wirless_ID){				//通讯模块驱动激活
						...
						case MID_TRANS_Zigbee	:wirelessThread_Active(1);	break;////数据传输驱动进程函数，包括有wifi及zigbee；
					...
					delay_ms(20);
					osSignalSet (tid_USARTWireless_Thread, WIRLESS_THREAD_EN);
					...
					switch(Moudle_GTA.Wirless_ID){				//通讯模块驱动终止
						...
						case MID_TRANS_Zigbee	:	wirelessThread_Active(0);break;
