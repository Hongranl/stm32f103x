 ...
 * @Module:     infraTrans
 * @Author:     RanHongLiang
 * @Date:       2019-07-03 09:00:02
 * @Description: 
 *————红外学习及转发模块，采用标准IEEE中的NEC协议，学习后的按键值都放入flash
 *---------------------------------------------------------------------------*/

#include "infraTrans.h" //红外学习及转发驱动进程函数；
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
/*---------------------------------------------------------------------------
 * @Description:矩阵按键扫描，消抖除延时，获取有效按键值
 * @Param:      无
 * @Return:     返回按键值
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
 * @Description:读取红外载波信息，判断有无，存储红外载波
 * @Param:      无
 * @Return:     无
 *---------------------------------------------------------------------------*/
void usr_sigin(void)
...
/*---------------------------------------------------------------------------
 * @Description:红外学习模块外部信息，按键扫描、显示同步、同步控制
 * @Param:      无
 * @Return:     无
 *---------------------------------------------------------------------------*/
void keyIFR_Thread_umdScan(const void *argument)
{
	...
	for (;;)
	{
		...
		{ //等待消息指令
			...
			do
			{
				status = osPoolFree(IFR_pool, rptr);
			} while (status != osOK); //内存释放
			rptr = NULL;
		}

		ifrKey_temp = keyIFR_Scan();
		if (ifrvalK_NULL != ifrKey_temp)
		{
			ifrCM_Attr.VAL_KEY = ifrKey_temp;
		}
		    ...
		    ifrCM_Attr_tempDP.STATUS != ifrCM_Attr.STATUS)
		{
			...
			do
			{
				mptr = (IFR_MEAS *)osPoolCAlloc(IFR_pool);
			...
			gb_Exmod_key = true;
			gb_databuff[0] = mptr->Mod_addr;
			gb_databuff[1] = mptr->VAL_KEY;
			gb_databuff[2] = mptr->STATUS;
...
/*---------------------------------------------------------------------------
 * @Description:红外遥控线程，红外发送、红外接收信息存储
 * @Param:      无
 * @Return:     无
 *---------------------------------------------------------------------------*/
void keyIFR_Thread(const void *argument)
{
	...
	if ((ifrDevMOUDLE_ID != ifrDevMID_video) &&
	    (ifrDevMOUDLE_ID != ifrDevMID_audio))
		ifrDevMOUDLE_ID = ifrDevMID_audio;
	...
	for (;;)
	{
		...
		if (evt.status == osEventMessage)
		{
			...
			do
			{
				status = osPoolFree(memID_IFRsigK_pool, rptr_sigK);
			} while (status != osOK); //内存释放
			rptr_sigK = NULL;
		}
		...
		if (ifrCM_Attr.VAL_KEY != ifrvalK_NULL)
		{
			...
			else
			{
				...
				if ((HTtab[0] + HTtab[1] + LTtab[0] + LTtab[1]) > 65000)
				{
					ifrCM_Attr.VAL_KEY = ifrvalK_NULL;
					continue;
				} //取样，读到非法溢出值（未经存储过的值），无法使用
