 ...
 * @Module:     gasMS
 * @Author:     RanHongLiang
 * @Date:       2019-07-03 08:18:25
 * @Description: 
 *————燃气检测模块
 *	该传感器能检测的浓度为0.2%-5%，严谨的说这里应该采用相对浓度，或者电压转换为其参考浓度
 * ---------------------------------------------------------------------------*/

#include "gasMS.h" //燃气检测驱动进程函数；
...
/*---------------------------------------------------------------------------
 * @Description:可燃气体的报警电平检测IO，既是高电平报警，低电平解除报警
 * @Param:      无
 * @Return:     无
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------
 * @Description:可燃气体的电压检测IO，ADC转换引脚
 * @Param:      无
 * @Return:     无
 *---------------------------------------------------------------------------*/
...
void gasMS_Thread(const void *argument)
{
	...
	for (;;)
	{
		...
		{ //等待消息指令
			...
			do
			{
				status = osPoolFree(gasMS_pool, rptr);
			} while (status != osOK); //内存释放
			rptr = NULL;
		}
		//这里 的浓度为0.2%-5%，所以使用100%为错误！故此更改为电压值
		sensorData.anaDAT = (gasGet_Adc_Average(0, 10) / 41); //数据采集
		...
		if (UPLD_EN)
		{
			...
			gb_databuff[1] = sensorData.anaDAT;
		    ...
		    Data_tempDP.VAL != sensorData.VAL)
		{
			...
			do
			{
				mptr = (gasMS_MEAS *)osPoolCAlloc(gasMS_pool);
			} while (mptr == NULL);
