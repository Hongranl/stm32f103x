...
void USART2_dev95HF_Init(void){
    ...
    Driver_USART2.Send("i'm usart2 with 95HF.\r\n", 23);
	osDelay(20);
} 
...
/*----CD--------------- Drivers Initialization -------------------------------*/
	USART2_dev95HF_Init();
		...
		Driver_USART2.Send("step 1 complete.\r\n", 18);
		osDelay(20);
  ...
  /* Led Configuration */
	...
	ConfigManager_HWInit();
		...
		Driver_USART2.Send("step 2 complete.\r\n", 18);
		osDelay(20);
	...
	Driver_USART2.Send("RFID_95HF Initialize completed.\r\n", 33);
}
			...
			switch(TagType){
				...
				case TRACK_NFCTYPE1:{
						...
						if(memcmp (LastUIDFound, TagUID, 6)){	
							...
							sprintf(disp,"TT1:%s\r\n",UID);
							Driver_USART2.Send((uint8_t*)disp,strlen((const char*)disp));			
						}
						else{
