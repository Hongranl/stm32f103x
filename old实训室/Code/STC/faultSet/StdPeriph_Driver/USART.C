...
void Aryorg_Send1(u8 array[],u8 num){						//原封发送数组，长度num

	u8 loop;
	
	WR485;		//485发送
	for(loop = 0;loop < num;loop ++)TX1_write2buff(array[loop]);
	RD485;		//发送完毕即打开485接收
}

void Aryorg_Send2(u8 array[],u8 num){						//原封发送数组，长度num

	u8 loop;
	
	for(loop = 0;loop < num;loop ++)TX2_write2buff(array[loop]);
}
