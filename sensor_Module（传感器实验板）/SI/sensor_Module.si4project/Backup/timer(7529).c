...
static u32 led_delay; 
extern  bool gb_status_net ;//入网请求状态
//通用定时器3中断初始化
...
void TIM2_IRQHandler(void)   //TIM3中断
{
	if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET)  //检查TIM3更新中断发生与否
	{
		...
		RS485_V_I_timeout_fution();  //1ms  ONCE 
		 android_timeout_fution() ;
		 zigbee_uart_timeout_fution() ;
		...
		if(gb_status_net)//入网状态灯
		{
			if((led_delay%200) == 0 )
			{
				Led_Zigbee_OK = ~Led_Zigbee_OK;
				//led_delay = 0;
			}
			else if(led_delay > 5000)
			{
				gb_status_net = false;
				led_delay = 0;
				Led_Zigbee_OK = 0;
...
void TIM1_Init(void)
{
	...
	TIM_BaseInitStruct.TIM_Prescaler = 72-1;//预分频值	72分频，频率为1MHz
	TIM_BaseInitStruct.TIM_CounterMode = TIM_CounterMode_Up;//向上计数
	TIM_BaseInitStruct.TIM_Period = 0xffff;//以1Mhz的频率计数 //= 1000-1;自动重装载值 溢出时间为 1ms(1KHz) 
	TIM_BaseInitStruct.TIM_ClockDivision = TIM_CKD_DIV1;//时钟分频因子 = 1;不分频
