...
#include "timer.h"
  ...
  Uart_struct    UART3_zigbee_COMM_st;
    
/*********************************************************************
 *  LOCAL  FUNC
 */
 void   uart1_init(u32 bound);	  //打印，调试 
 void   uart2_init(u32 bound);	//与RS485_COMM 串口交互
...
{   
   ...
   //USART 初始化设置
	...
	MYDMA_Config( UART2_DMA_TX_CH,(u32)&USART2->DR,(u32)UART2_RS485_COMM_st.SDcun,UART_SD_LEN ,DMA_DIR_PeripheralDST,DMA_Mode_Normal);//DMA1通道4,外设为串口1,存储器为UART1_SDBuff,长度USART1_SD_LEN. 
	DMA_Cmd(UART2_DMA_TX_CH, DISABLE);// 没数据发不要开启这条，一开启就会发出数据
	MYDMA_Config( UART2_DMA_RX_CH,(u32)&USART2->DR,(u32)UART2_RS485_COMM_st.RXcun,UART_RX_LEN ,DMA_DIR_PeripheralSRC,DMA_Mode_Normal);//DMA1通道4,外设为串口1,存储器为UART1_SDBuff,长度USART1_SD_LEN. 
	DMA_Cmd(UART2_DMA_RX_CH, ENABLE);// 
	 
	DMA_ITConfig(UART2_DMA_TX_CH,DMA_IT_TC,  ENABLE); //开DMA传送完中断
	...
	memset((u8*)&UART2_RS485_COMM_st, FALSE,sizeof(UART2_RS485_COMM_st));//标志位  全部为 FALSE
...
{   
  ...
  MYDMA_Config( UART3_DMA_TX_CH,(u32)&USART3->DR,(u32)UART3_zigbee_COMM_st.SDcun,UART_SD_LEN ,DMA_DIR_PeripheralDST,DMA_Mode_Normal);//DMA1通道4,外设为串口1,存储器为UART1_SDBuff,长度USART1_SD_LEN. 
  DMA_Cmd(UART3_DMA_TX_CH, DISABLE);// 没数据发不要开启这条，一开启就会发出数据
  MYDMA_Config( UART3_DMA_RX_CH,(u32)&USART3->DR,(u32)UART3_zigbee_COMM_st.RXcun,UART_RX_LEN ,DMA_DIR_PeripheralSRC,DMA_Mode_Normal);//DMA1通道4,外设为串口1,存储器为UART1_SDBuff,长度USART1_SD_LEN. 
  DMA_Cmd(UART3_DMA_RX_CH, ENABLE);// 
		
  DMA_ITConfig(UART3_DMA_TX_CH,DMA_IT_TC,  ENABLE); //开DMA传送完中断
  ...
  USART_Cmd(USART3, ENABLE);                    //使能串口 
	...
	memset((u8*)&UART3_zigbee_COMM_st, FALSE,sizeof(UART3_zigbee_COMM_st));//标志位  全部为 FALSE
   
  UART3_zigbee_COMM_st.ALL_tc_flg = TRUE;
...
void disable_all_uart_interupt( void  )
{
	...
	DMA_ITConfig(USART3_DMA_TX_CH,DMA_IT_TC,DISABLE);      
	USART_ITConfig(USART2, USART_IT_IDLE, DISABLE);  
	DMA_ITConfig(USART2_DMA_TX_CH,DMA_IT_TC,DISABLE);	     
}
...
void enable_all_uart_interupt( void  )
{
	...
	DMA_ITConfig(USART3_DMA_TX_CH,DMA_IT_TC,ENABLE);   
	DMA_ITConfig(USART2_DMA_TX_CH,DMA_IT_TC,ENABLE); 
