...
return crc;
 ...
 {  
    ...
    { 
         ...
         DMA_Cmd(UART2_DMA_RX_CH, DISABLE);// 禁用  避免出错
         Data_Len=UART_RX_LEN- DMA_GetCurrDataCounter(UART2_DMA_RX_CH);  
		 ...
		 {  
          ...
          memset(UART2_android_st.RXcun, 0,i+2);
					 ...
					 UART2_DMA_RX_CH->CNDTR=UART_RX_LEN; //  重新赋值计数值，必须大于等于最大
...
{   
    ...
    {   
        ...
        DMA_Cmd(UART2_DMA_TX_CH, DISABLE);// 禁用  避免出错 或发送
...
void debug_uart2_Sendbuff(u8 number)
{
	u8 i;
	printf("uart2发送缓存--数据:\n");
	
	for(i = 0; i < UART2_android_st.SDBuff_len[number]; i++)
		printf("%3.2x",UART2_android_st.SDBuff[number][i]);
			  ...
			  if( android_CHECK_BUF_VALID(UART2_android_st.SDBuff[cun_sd_lun])==TRUE)//如果有数据
				 {
						...
						//debug_uart2_Sendbuff(cun_sd_lun);		 
						...
						memset(UART2_android_st.SDBuff[cun_sd_lun], FALSE,10 ); //清空
								
						MYDMA_Config( UART2_DMA_TX_CH,(u32)&USART2->DR,(u32)UART2_android_st.SDcun,UART_SD_LEN,DMA_DIR_PeripheralDST,DMA_Mode_Normal);//DMA1通道4,外设为串口1,存储器为UART1_SDBuff,长度USART1_SD_LEN. 
								 
		      			MYDMATX_Enable(UART2_DMA_TX_CH, UART2_android_st.SDBuff_len[cun_sd_lun] );		
...
{     u8  crc=0, i=0,cun_sd_lun;
	...
	for(i=0;i<USART_TXRX_PK_NUM ;i++)
	{  
		if( android_CHECK_BUF_VALID(UART2_android_st.SDBuff[cun_sd_lun]) == FALSE)//如果没有数据
		{ 
			...
			crc= android_crc8(&UART2_android_st.SDBuff[cun_sd_lun][0],3+sizeof(TRAN_D_struct)+len+2);
