...
uint32_t Get_Adc_Average(uint8_t ch,uint8_t times)
{
	...
	for(t=0;t<times;t++)
	{
		temp_val += ((Get_Adc(0) *10*1000)/ (4096*3)) ;
		delay_ms(2);
	}
...
void RelaysoutletCM_Thread(const void *argument){
	...
	for(;;){
		...
		actuatorData.Current = ;//Vi (mv) =data* 3.3* 1000 /4096
		//continue;
		if (actuatorData.Current > 2300)//有2500mV的直流电压，去除掉
			{
			actuatorData.Current = (actuatorData.Current - 2300);//185mv/A ---> I(A) =  Vi/185
			}
		else
			{
