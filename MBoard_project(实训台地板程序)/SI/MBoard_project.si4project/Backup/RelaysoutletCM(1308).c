...
void RelaysoutletCM_Thread(const void *argument){
	...
	for(;;){
		...
		actuatorData.Current =((Get_Adc_Average(0,10) *10*1000)/ (4096*3))  ;//Vi (mv) =data* 3.3* 1000 /4096
		//continue;
		if (actuatorData.Current > 2500)//有2500mV的直流电压，去除掉
			{
			actuatorData.Current = (actuatorData.Current - 2500) /185;//185mv/A ---> I(A) =  Vi/185
			}
		else
			{
