...
void UsartRx_Thread(const void *argument	)
{
	...
	while(1)
	{
		...
		Driver_USART2.Receive(recvbuff,23);
		osSignalWait(0x02, osWaitForever);
		recvbuff[25] = Driver_USART2.GetRxCount();
	...
	if(led2_status != led2_b)
		{
			...
			while(1)
			{
				...
				if (led2_status == led2_b_flash && i++ < 100)
				{
	...
	switch (EXmodID)
		{
			...
			case MID_EXEC_RELAYSLIGHT:{
						...
						if (evt.status == osEventMessage) {		//等待消息指令
							...
							memcpy(sendbuff+16+updataTRAN.data_len, rptr->relay_con, 1);//17+updataTRAN.data_len-1
							sendbuff[20] = MID_EXEC_RELAYSLIGHT;
							osDelay(20);
...
//	Moudle_GTA.Wirless_ID = 0xAA;   			/****调试语句*****/
	...
	for(;;){
		...
		if( led2_status == led2_b && gb_Exmod_key )
		{
			gb_Exmod_key = false;
...
/*************↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓无线数据接收，处理推送至驱动级进程↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓*********************/
		...
		if(recvbuff[0] == 0xaa && recvbuff[1] == 0xaa)
		{
			...
			crc = TRAN_crc8(recvbuff,17+recv_tranbuff.data_len+2);
