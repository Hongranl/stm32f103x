...
//							}
			...
			case MID_EXEC_SOURCE:{
					...
					sourceCM_MEAS *rptr;
						...
						float power_i ;
						evt = osMessageGet(MsgBox_DPsourceCM, 10);
						if (evt.status == osEventMessage || gb_power_kwh > 1){
							beeps(2 );
							rptr = evt.value.p;
							/*显示部分程序↓↓↓↓↓↓↓*/
							 power_p =  gb_power_kwh/10;
							 power_v =  gb_power_v/10;
							 power_i = gb_power_A/ 1000;
							
							gb_power_kwh = 0;
							LCD_1_44_ClearS(BLACK,0,50,128,75);
							sprintf(disp,"%.1f KWH",power_p);
							Show_Str(40,50,GREEN,BLACK,(uint8_t *)disp,24,1);
							
							LCD_1_44_ClearS(BLACK,0,75,127,127);
							sprintf(disp,"%.1f V",power_v);
							Show_Str(40,75,GREEN,BLACK,(uint8_t *)disp,24,1);
							
							LCD_1_44_ClearS(BLACK,0,100,127,127);
							sprintf(disp,"%.2f A",power_i);
							Show_Str(40,100,GREEN,BLACK,(uint8_t *)disp,24,1);
