	...
	if( UART2_RS485_COMM_st.ALL_tc_flg == TRUE )//DMA传送完毕
	{
		...
		{  
			if( Rs485_COMM_CHECK_BUF_VALID(UART2_RS485_COMM_st.SDBuff[cun_sd_lun]) == TRUE)//如果有数据
			{
				...
				memset(UART2_RS485_COMM_st.SDBuff[cun_sd_lun], FALSE, UART2_RS485_COMM_st.SDBuff_len[cun_sd_lun] ); //把数据COPY到	 
				//	LED1 = !LED1;   //  输出
				RS485_CON2 = 1;	
				RS485_CON2 = 1;	
				LED1 = 0;
				...
				USART_DMACmd(USART2,USART_DMAReq_Tx,ENABLE); //使能串口2的DMA发送 接收  
