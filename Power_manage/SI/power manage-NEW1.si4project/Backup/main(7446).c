...
void Updata_J_status(char status)
{
	...
	TRAN_info.dest_addr[0] = (u8) (DTN_android << 8) ;
	TRAN_info.dest_addr[1] = (u8) DTN_android >> 8 ;
	TRAN_info.dest_addr[2] = 0x00;
	TRAN_info.dest_addr[3] = 0x00;
...
{  
	...
	*/	
	...
	{ 
		...
		Rs485_COMM_uart_fuc() ;
		if(gb_is_DMA_send_OK == 1 && gb_time_count_send < 30)
		{
			IWDG_Feed();
		}
