...
/**
  * @brief  Write_Flash_J_status,对内部FLASH进行写入继电器状态
  * @param  0 ；1
  * @retval None
  */
	...
	uint32_t NbrOfPage = 0x00;			//记录写入多少页
	 ...
	 /* 解锁 */
	  ...
	  for(EraseCounter = 0; (EraseCounter < NbrOfPage) ; EraseCounter++)
	  {
	    ...
	    FLASH_ProgramWord(WRITE_START_ADDR, Data);
...
/**
  * @brief  Read_Flash_J_status,对内部FLASH进行读取继电器状态
  * @param  None
  * @retval 0\1
  */
...
{
	 return  (*(__IO uint32_t*) WRITE_START_ADDR) == 0 ? 0 : 1 ;
}
