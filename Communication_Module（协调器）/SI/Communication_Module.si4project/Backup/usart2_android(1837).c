          ...
          for(i=0;i<USART_TXRX_PK_NUM ;i++)
				 {  
                    ...
                    crc= android_crc8(&UART2_android_st.SDBuff[cun_sd_lun][0],3+sizeof(TRAN_D_struct)+len+2);
									...
									/*
									   sprintf(buff ,"from maincon to android :"); 
									
								  	UART1_SD_load_buf((u8*)buff, strlen(buff));
									  memset(buff,0,strlen(buff));
									 
									  for(nx=0;nx < UART2_android_st.SDBuff_len[cun_sd_lun];nx++ )
									  {
									   offset1+= sprintf(buff+offset1  ,"%02x+" ,UART2_android_st.SDBuff[cun_sd_lun][nx] ); 
									  }
									  UART1_SD_load_buf((u8*)buff, strlen(buff));  
		      ...
		      // enable_all_uart_interupt();				 
				  DMA_ITConfig(android_DMA_TX_CH,DMA_IT_TC,ENABLE);  //临界点
				  USART_ITConfig(USART2, USART_IT_IDLE, ENABLE); 
