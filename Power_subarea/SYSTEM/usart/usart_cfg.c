...
#include "timer.h"
  ...
  Uart_struct    UART3_RS485_V_I_st; 
/*********************************************************************
 *  LOCAL  FUNC
 */
 void   uart1_init(u32 bound);	  //打印，调试 
 void   uart3_init(u32 bound);	//与RS485_COMM 串口交互
...
//bound:波特率
 void uart1_init(u32 bound){
  ...
  USART_ITConfig(USART1, USART_IT_IDLE, ENABLE);  //串口1空闲中断 
   ...
   MYDMA_Config( UART1_DMA_RX_CH,(u32)&USART1->DR,(u32)UART1_zigbee_st.RXcun,UART_RX_LEN,DMA_DIR_PeripheralSRC,DMA_Mode_Normal);//DMA1通道4,外设为串口1,存储器为UART1_SDBuff,长度USART1_SD_LEN. 
  ...
  USART_Cmd(USART1, ENABLE);                    //使能串口 
	...
	memset((u8*)&UART1_zigbee_st, FALSE,sizeof(UART1_zigbee_st));//标志位  全部为 FALSE
...
{   
  ...
  MYDMA_Config( RS485_COMM_DMA_TX_CH,(u32)&USART3->DR,(u32)UART3_RS485_V_I_st.SDcun,UART_SD_LEN ,DMA_DIR_PeripheralDST,DMA_Mode_Normal);//DMA1通道4,外设为串口1,存储器为UART1_SDBuff,长度USART1_SD_LEN. 
  DMA_Cmd(RS485_COMM_DMA_TX_CH, DISABLE);// 没数据发不要开启这条，一开启就会发出数据
  MYDMA_Config( RS485_COMM_DMA_RX_CH,(u32)&USART3->DR,(u32)UART3_RS485_V_I_st.RXcun,UART_RX_LEN ,DMA_DIR_PeripheralSRC,DMA_Mode_Normal);//DMA1通道4,外设为串口1,存储器为UART1_SDBuff,长度USART1_SD_LEN. 
  DMA_Cmd(RS485_COMM_DMA_RX_CH, ENABLE);// 
		 
  DMA_ITConfig(RS485_COMM_DMA_TX_CH,DMA_IT_TC,  ENABLE); //开DMA传送完中断
  ...
  USART_Cmd(USART3, ENABLE);                    //使能串口 
	...
	memset((u8*)&UART3_RS485_V_I_st, FALSE,sizeof(UART3_RS485_V_I_st));//标志位  全部为 FALSE
