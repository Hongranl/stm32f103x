...
{
	//if(CarSetingTurn != TURN_NULL)  return;   //add by  5-25
	               
	if(carStopFlag||carStopFlag2||Traffic_red)	// 停车
	{
		if(CarSetTurnTimer == 0)//刷卡盲走不停车
		{
			TIM_SetCompare1(TIM1,0);//PWM
			TIM_SetCompare4(TIM1,0);//PWM
			curSpeed = 0;
		}
	}
	else
	{
		if((discRanging>5)&&(discRanging<150))	// 100mm 超声波检测
		{
			TIM_SetCompare1(TIM1,0);//PWM
			TIM_SetCompare4(TIM1,0);//PWM
			curSpeed = 0;
		...
		else
		{
			...
			{			
				...
				case SERVO_TURN_L2:
					#if CAR_NUMBER == 3
				    SetMotorF_PWMDuty(600);	
					#elif  CAR_NUMBER == 1
					SetMotorF_PWMDuty(580);	//600
					#else
					SetMotorF_PWMDuty(650);		//650  
				    #endif
					break;
				
				default:
					#if CAR_NUMBER == 3 
				    SetMotorF_PWMDuty(260+speedOffest*60);	  //300
				    #elif  CAR_NUMBER == 1
					SetMotorF_PWMDuty(260+speedOffest*60);
				    #else
				    SetMotorF_PWMDuty(360+speedOffest*60);	//360
...
void LED_Control(void)
{
	...
	{
		case 1:	
			    if(carStopFlag||carStopFlag2||Traffic_red)
				{
					LED0 = 1;
					LED1 = 1;
				}
