...
/*静态局部变量*/
static uint32_t SHT11_Time = 0;		//用于监控两次测量之间的时间，必须大于1000(ms)
/**************/
/*************************************************************
  Function   ：SHT10_Dly  
  Description：SHT10时序需要的延时  72MHz/8MHz
  Input      : none        
  return     : none    
*************************************************************/
void SHT10_Dly(void)
...
//	GPIO_Init(SHT10_DATA_PORT,SHT10_DATA_PIN,GPIO_MODE_OUT_PP_HIGH_FAST);
...
/*************************************************************
  ...
  return     : u8 0-失败 1-成功   
*************************************************************/
u8 SHT1X_GetValue(u16*temp_val,u16*humi_val,float*temp_val_real,float*humi_val_real)
{
	u8  err = 0, checksum = 0;
...
void SHT10_DATAIn(void)
{
	...
	GPIOD->CR1 &=~ GPIO_PIN_2;//输入模式

	//GPIO_Init(SHT10_DATA_PORT,SHT10_DATA_PIN,GPIO_MODE_IN_FL_NO_IT);
//        GPIO_InitTypeDef GPIO_InitStructure;
//        //PD0 DATA 浮动输入        
//        GPIO_InitStructure.GPIO_Pin = SHT10_DATA_PIN;
//        GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
